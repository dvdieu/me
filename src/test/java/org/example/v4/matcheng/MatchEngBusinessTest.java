...
public class MatchEngBusinessTest extends BaseMatchEngTest {
    ...
    void shouldMatchSellOrderAndTriggerStopBook() {
        ...
        placeOrderAndValidate(matchEng, Order.createStopOrder(19, USER_BUY, BUY, STOP_LIMIT, GTC, 97, 98, 3));
        placeOrderAndValidate(matchEng, Order.createStopOrder(20, USER_SELL, SELL, STOP_MARKET, GTC, 96, 0, 3));
        placeOrderAndValidate(matchEng, Order.createStopOrder(21, USER_SELL, BUY, STOP_LIMIT, GTC, 98, 98, 4));

        Order incoming = Order.createStandardOrder(1000, -1, SELL, MARKET, GTC, 0, 110);
        incoming.stopAfterFirstCommand = true;
        placeOrderAndValidate(matchEng, incoming);
        ...
        assertThat(incoming.matcherTradeEvents.size(), is(14));
        checkEventTrade(incoming, 0, 102, 98, 13);
        checkEventTrade(incoming, 1, 21, 98, 4);
        checkEventTrade(incoming, 2, 103, 97, 14);
        checkEventTrade(incoming, 3, 3, 97, 3);
        checkEventTrade(incoming, 4, 6, 97, 2);
        checkEventTrade(incoming, 5, 11, 97, 3);
        checkEventTrade(incoming, 6, 19, 97, 3);
        checkEventTrade(incoming, 7, 104, 96, 10);
        checkEventTrade(incoming, 8, 105, 95, 18);
        checkEventTrade(incoming, 9, 106, 94, 7);
        checkEventTrade(incoming, 10, 107, 93, 12);
        checkEventTrade(incoming, 11, 108, 92, 9);
        checkEventTrade(incoming, 12, 109, 91, 10);
        checkEventTrade(incoming, 13, 110, 90, 2);
    }
